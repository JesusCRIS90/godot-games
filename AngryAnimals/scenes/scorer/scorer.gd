...
func _ready():
	...
	SignalManager.on_attempt_made.connect(slot_on_attempt_made)
	SignalManager.on_cup_destroyed.connect(slot_on_cup_destroyed)


func slot_on_attempt_made() -> void:
	_attempts += 1
	SignalManager.on_score_updated.emit(_attempts)
	
func slot_on_cup_destroyed() -> void:
	_cups_hit += 1
